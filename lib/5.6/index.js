...
exports.loadDependentSymbols = function () {
    ...
    _library._IO_marker = Struct({
        "_next": ref.refType(_library._IO_marker),
        "_pos": 'int',
        "_sbuf": ref.refType(_library._IO_FILE)
    });
    
    _library._IO_FILE = Struct({
        "_IO_backup_base": 'char' ? ref.refType('char') : 'pointer',
        "_IO_buf_base": 'char' ? ref.refType('char') : 'pointer',
        "_IO_buf_end": 'char' ? ref.refType('char') : 'pointer',
        "_IO_read_base": 'char' ? ref.refType('char') : 'pointer',
        "_IO_read_end": 'char' ? ref.refType('char') : 'pointer',
        "_IO_read_ptr": 'char' ? ref.refType('char') : 'pointer',
        "_IO_save_base": 'char' ? ref.refType('char') : 'pointer',
        "_IO_save_end": 'char' ? ref.refType('char') : 'pointer',
        "_IO_write_base": 'char' ? ref.refType('char') : 'pointer',
        "_IO_write_end": 'char' ? ref.refType('char') : 'pointer',
        "_IO_write_ptr": 'char' ? ref.refType('char') : 'pointer',
        "__pad1": 'void' ? ref.refType('void') : 'pointer',
        "__pad2": 'void' ? ref.refType('void') : 'pointer',
        "__pad3": 'void' ? ref.refType('void') : 'pointer',
        "__pad4": 'void' ? ref.refType('void') : 'pointer',
        "__pad5": 'ulong',
        "_chain": ref.refType(_library._IO_FILE),
        "_cur_column": 'ushort',
        "_fileno": 'int',
        "_flags": 'int',
        "_flags2": 'int',
        "_lock": 'void' ? ref.refType('void') : 'pointer',
        "_markers": ref.refType(_library._IO_marker),
        "_mode": 'int',
        "_offset": 'long',
        "_old_offset": 'long',
        "_shortbuf": RefArray('char', 1),
        "_unused2": RefArray('char', 20),
        "_vtable_offset": 'char'
    });
    ...
    _library.__sigset_t = Struct({
        "__val": RefArray('ulong', 16)
    })
    
    _library.__jmp_buf_tag = Struct({
        "__jmpbuf": _library.__jmp_buf,
        "__mask_was_saved": 'int',
        "__saved_mask": _library.__sigset_t
    });
